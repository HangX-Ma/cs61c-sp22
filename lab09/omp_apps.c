...
void v_add_optimized_chunks(double* x, double* y, double* z) {
    ...
    #pragma omp parallel
    {
        int elem_idx, thread_idx = thread_num * chunk_size;
        for (int idx = 0; idx < chunk_size; ++idx) {
            elem_idx = thread_idx + idx;
            z[elem_idx] = x[elem_idx] + y[elem_idx];
        }
        // tail
        if (thread_idx == num_threads * chunk_size) {
            for (; thread_idx < ARRAY_SIZE; thread_idx++) {
                z[thread_idx] = x[thread_idx] + y[thread_idx];
            }
        }
    }
}
